...
train_envs = ts.env.VectorEnv([lambda: NetEnv() for _ in range(20)])
test_envs = ts.env.VectorEnv([lambda: NetEnv() for _ in range(4)])

net = DQNNet(3*136, 3, device).to(device)
...
result = ts.trainer.offpolicy_trainer(
    ...
    episode_per_test=2, batch_size=64,
